...
int ftplib::Connect(const char *host) {
    ...
    if (setsockopt(sControl, SOL_SOCKET, SO_SNDTIMEO, SETSOCKOPT_OPTVAL_TYPE &timeout, sizeof(timeout)) == -1) {
        perror("setsockopt");
        net_close(sControl);
        return 0;
    }
    timeout = {timeout_seconds, 0};
    if (setsockopt(sControl, SOL_SOCKET, SO_RCVTIMEO, SETSOCKOPT_OPTVAL_TYPE &timeout, sizeof(timeout)) == -1) {
        perror("setsockopt");
        net_close(sControl);
        return 0;
    }
...
int ftplib::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd) {
    ...
    struct timeval timeout = {timeout_seconds, 0};
    if (setsockopt(sData, SOL_SOCKET, SO_SNDTIMEO, SETSOCKOPT_OPTVAL_TYPE &timeout, sizeof(timeout)) == -1) {
        perror("setsockopt");
        net_close(sData);
        return -1;
    }
    timeout = {timeout_seconds, 0};
    if (setsockopt(sData, SOL_SOCKET, SO_RCVTIMEO, SETSOCKOPT_OPTVAL_TYPE &timeout, sizeof(timeout)) == -1) {
        perror("setsockopt");
        net_close(sData);
        return -1;
    }
...
void ftplib::SetTimeout(unsigned int timeout_seconds) {
